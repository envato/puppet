...
on master, puppet("module upgrade nginx"), :acceptable_exit_codes => [1] do
  assert_output <<-OUTPUT
    ...
    STDERR>   Module 'nginx' does not exist on https://forge.puppet.com\e[0m
  OUTPUT
end
...
on master, puppet("module upgrade notpmtacceptance-unicorns"), :acceptable_exit_codes => [1] do
  assert_output <<-OUTPUT
    ...
    STDERR>   Module 'notpmtacceptance-unicorns' does not exist on https://forge.puppet.com\e[0m
  OUTPUT
end
...
on master, puppet("module upgrade pmtacceptance-java --version 2.0.0"), :acceptable_exit_codes => [1] do
  assert_output <<-OUTPUT
    ...
    STDERR>   No version matching '2.0.0' exists on https://forge.puppet.com\e[0m
  OUTPUT
end
